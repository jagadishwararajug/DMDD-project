...
SET SERVEROUT ON;
------------------------------------------------UNIVERSITY UPDATE-----------------------------------------------------------------
CREATE OR REPLACE PROCEDURE UPDATE_UNIVERSITY_TABLE(uni_name in VARCHAR2, column_name in VARCHAR2, column_val in VARCHAR2)
AS
...
BEGIN
        ...
        select UNIVERSITY_ID into uniId FROM UNIVERSITY where UNIVERSITY_NAME=uni_name;
        DBMS_OUTPUT.PUT_LINE(uniId);
        update_query := 'UPDATE UNIVERSITY set '|| col_name || '=''' || col_val ||  ''' where UNIVERSITY_NAME=''' || uni_name || ''' and UNIVERSITY_ID=' || uniId; 
        DBMS_OUTPUT.PUT_LINE(update_query);
        DBMS_OUTPUT.PUT_LINE('-------------------------------------------------------------------------------------------------------------------');
        EXECUTE IMMEDIATE  'UPDATE UNIVERSITY set '|| col_name || '=''' || col_val ||  ''' where UNIVERSITY_NAME=''' || uni_name || ''' and UNIVERSITY_ID=' || uniId; 
COMMIT;
EXCEPTION WHEN others then
...
execute UPDATE_UNIVERSITY_TABLE('Northeastern University', 'UNIVERSITY_STREET', 'Hunting St');
