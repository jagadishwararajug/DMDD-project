...
begin
-- table Users
SELECT count(*) into tableCount FROM user_tables where table_name = 'USERS';
IF(tableCount > 0)
THEN
...
-- table UNIVERSITY
SELECT count(*) into tableCount FROM user_tables where table_name = 'UNIVERSITY';
IF(tableCount > 0)
THEN
...
SELECT count(*) into tableCount FROM user_tables where table_name = 'Vaccine_Details';
IF(tableCount > 0)
THEN
    DBMS_OUTPUT.PUT_LINE('Vaccine_Details Table already exists');
Else
    execute immediate 'CREATE Vaccine_Details (
